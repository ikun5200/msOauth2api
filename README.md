...
| `CLIENT_ID` | 使用 `/api/auth/login` 时必填 | Azure App Registration 的 Application (client) ID。 |
| `TENANT_ID` | 否 | 微软租户 ID，默认可使用 `common`。 |
| `REDIRECT_URI` | 使用 `/api/auth/login` 时必填 | OAuth2 回调地址。使用内置取 Token 页面时填 `https://your-domain.com/token.html`。 |
...
本版本已内置标准 PKCE 登录流程，可直接打开 `https://your-vercel-app.vercel.app/token.html` 获取 `refresh_token`。

Azure Portal 中建议配置：

- Platform: Mobile and desktop applications
- Redirect URI: `https://your-domain.com/token.html`
- 勾选 Allow public client flows
- Delegated permissions: `Mail.Read`, `Mail.ReadWrite`, `Mail.Send`, `offline_access`, `openid`, `profile`

也可以直接访问 `/api/auth/login` 开始授权，授权完成后 `token.html` 会调用 `/api/auth/callback` 使用授权码换取 `access_token` 和 `refresh_token`。
...
- **方法**: `GET` 或 `POST`
- **URL**: `/api/token-info`
- **描述**: 检测当前 token 支持 Graph API 还是 IMAP，并返回可用功能。
- **参数说明**:
  - `refresh_token` (必填): 用于身份验证的 refresh_token。
  - `client_id` (必填): 客户端 ID。
  - `email` (可选): 邮箱地址。
...
- **方法**: `GET`
- **URL**: `/api/process-inbox` 或 `/api/clear-inbox`
- **描述**: 清空收件箱，优先使用 Microsoft Graph，不支持时回退 IMAP。
- **参数说明**:
  - `refresh_token` (必填): 用于身份验证的 refresh_token。
  - `client_id` (必填): 客户端 ID。
  - `email` (必填): 邮箱地址。
...
- **方法**: `GET`
- **URL**: `/api/process-junk` 或 `/api/clear-junk`
- **描述**: 清空垃圾箱，优先使用 Microsoft Graph，不支持时回退 IMAP。
- **参数说明**:
  - `refresh_token` (必填): 用于身份验证的 refresh_token。
  - `client_id` (必填): 客户端 ID。
  - `email` (必填): 邮箱地址。
...
- **方法**: `GET` 或 `POST`
- **URL**: `/api/send-mail`
- **描述**: 使用 Microsoft Graph API 发送邮件。
- **参数说明**:
  - `refresh_token` (必填): 用于身份验证的 refresh_token。
  - `client_id` (必填): 客户端 ID。
  - `email` (必填): 发件人邮箱地址。
...
- **方法**: `GET` 或 `POST`
- **URL**: `/api/delete-mail`
- **描述**: 删除指定邮件，支持 Graph API 与 IMAP 回退。
- **参数说明**:
  - `refresh_token` (必填): 用于身份验证的 refresh_token。
  - `client_id` (必填): 客户端 ID。
  - `email` (必填): 邮箱地址。
  - `message_id` (必填): 要删除的邮件 ID。Graph 模式传 Graph `id`，IMAP 模式传邮件 `Message-ID`。
