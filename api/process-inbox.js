...
module.exports = async (req, res) => {
  ...
  try {
    ...
    if (graphApiResult.status && graphApiResult.scope.includes('https://graph.microsoft.com/Mail.ReadWrite')) {
      return await processFolderGraphApi(graphApiResult.access_token, 'inbox', res);
    }

    return await processFolderImap(refresh_token, client_id, email, 'INBOX', res);
  } catch (error) {
...
async function processFolderGraphApi(accessToken, folder, res) {
  try {
    ...
    if (messages.length === 0) {
      return res.json({ message: 'No Inbox emails found.', mode: 'graph', stats: { total: 0, deleted: 0, failed: 0 } });
    }
...
async function processFolderImap(refreshToken, clientId, email, folder, res) {
  try {
